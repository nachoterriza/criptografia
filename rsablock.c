...
int main()
{
    ...
    for(; mensaje[i] != '\n'; i++)
    {
       ...
       printf("Grabado %c en bloque[%i][%i] -> %c\n", mensaje[i], i/TAM_BLOQUE, i%TAM_BLOQUE, bloque[i/TAM_BLOQUE][i%TAM_BLOQUE]);
    }
    bloque[i/TAM_BLOQUE][i%TAM_BLOQUE] = '\n';
    for(int k=0; k<=i/TAM_BLOQUE; k++)
        for(int l=0; l<TAM_BLOQUE; l++)
        {
            if(bloque[k][l] == '\n')
            {
                printf("---FIN---\n");
    ...
    for(int k=0; k<=i/TAM_BLOQUE; k++)
    {
        ...
        gmp_printf("Convertido %c%c%c%c -> %Zd\n", bloque[k][0], bloque[k][1], bloque[k][2], bloque[k][3], texto[k]);
    }
